...
void desktop_settings_scene_happy_mode_on_enter(void* context) {
    ...
    dialog_ex_set_header(app->dialog_ex, "Счастливый режим", 64, 0, AlignCenter, AlignTop);
    dialog_ex_set_text(
        app->dialog_ex,
        "Пока режим включен,\nя не обижусь, что ты\nсо мной не играешь",
        64,
        ...
        AlignCenter,
        AlignCenter);
    dialog_ex_set_left_button_text(app->dialog_ex, settings.happy_mode ? "Выключить" : "Назад");
    dialog_ex_set_right_button_text(app->dialog_ex, settings.happy_mode ? "Оставить" : "Включить");
