...
static void render_callback(Canvas* canvas, void* ctx) {
    ...
    if(stats->butthurt <= 4) {
        mood = 0;
        snprintf(mood_str, sizeof(mood_str), "Настрой: супер");
    } else if(stats->butthurt <= 9) {
        mood = 1;
        snprintf(mood_str, sizeof(mood_str), "Настрой: норм");
    } else {
        mood = 2;
        snprintf(mood_str, sizeof(mood_str), "Настрой: злой");
    }
    ...
    snprintf(level_str, sizeof(level_str), "Уровень: %hu", stats->level);
    canvas_draw_str(canvas, 58, 12, my_name ? my_name : "Неизвестно");
