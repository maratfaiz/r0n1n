...
static void u2f_view_draw_callback(Canvas* canvas, void* _model) {
    ...
    if(model->display_msg == U2fMsgNotConnected) {
        ...
        canvas_draw_str_aligned(canvas, 128 / 2, 3, AlignCenter, AlignTop, "Подключи к ПК");
    } else if(model->display_msg == U2fMsgIdle) {
        canvas_draw_icon(canvas, 22, 15, &I_Connected_62x31);
        canvas_draw_str_aligned(canvas, 128 / 2, 3, AlignCenter, AlignTop, "Подключено!");
    } else if(model->display_msg == U2fMsgRegister) {
        elements_button_center(canvas, "OK");
        canvas_draw_icon(canvas, 22, 15, &I_Auth_62x31);
        canvas_draw_str_aligned(canvas, 128 / 2, 3, AlignCenter, AlignTop, "Регистрация: жми OK");
    } else if(model->display_msg == U2fMsgAuth) {
        elements_button_center(canvas, "OK");
        canvas_draw_icon(canvas, 22, 15, &I_Auth_62x31);
        canvas_draw_str_aligned(canvas, 128 / 2, 3, AlignCenter, AlignTop, "Вход: жми OK");
    } else if(model->display_msg == U2fMsgSuccess) {
        canvas_draw_icon(canvas, 22, 15, &I_Connected_62x31);
        canvas_draw_str_aligned(canvas, 128 / 2, 3, AlignCenter, AlignTop, "Вход выполнен!");
    } else if(model->display_msg == U2fMsgError) {
        canvas_draw_icon(canvas, 22, 15, &I_Error_62x31);
        canvas_draw_str_aligned(canvas, 128 / 2, 3, AlignCenter, AlignTop, "Ошибка сертификата");
    }
}
