...
static void infrared_progress_view_draw_callback(Canvas* canvas, void* _model) {
    ...
    elements_multiline_text_aligned(
        ...
        AlignCenter,
        AlignCenter,
        model->is_paused ? "Пауза" : "Отправка...");
    ...
    canvas_draw_str(canvas, buttons_x + 14, buttons_y + 8, model->is_paused ? "дальше" : "стоп");

    canvas_draw_icon(canvas, buttons_x + 1, buttons_y + 10, &I_Ok_btn_9x9);
    canvas_draw_str(
        canvas, buttons_x + 14, buttons_y + 17, model->is_paused ? "отпр. 1" : "пауза");
    ...
    if(model->is_paused) {
        ...
        canvas_draw_str(canvas, buttons_x + 14, buttons_y + 26, "выбрать");
    }
}
