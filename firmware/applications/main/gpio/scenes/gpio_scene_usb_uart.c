...
bool gpio_scene_usb_uart_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        dialog_ex_set_left_button_text(dialog, "Выйти");
        dialog_ex_set_right_button_text(dialog, "Остаться");
        dialog_ex_set_header(dialog, "Выйти из моста?", 22, 12, AlignLeft, AlignTop);
