...
static const char* flow_pins[] = {"Нет", "2,3", "6,7", "16,15"};
static const char* baudrate_mode[] = {"Хост"};
...
static const char* software_de_re[] = {"Нет", "4"};
...
void gpio_scene_usb_uart_cfg_on_enter(void* context) {
    ...
    item = variable_item_list_add(var_item_list, "Канал USB", 2, line_vcp_cb, app);
    ...
    item = variable_item_list_add(var_item_list, "Пины UART", 2, line_port_cb, app);
    ...
    item =
        variable_item_list_add(var_item_list, "RTS/DTR", COUNT_OF(flow_pins), line_flow_cb, app);
    variable_item_set_current_value_index(item, app->usb_uart_cfg->flow_pins);
    variable_item_set_current_value_text(item, flow_pins[app->usb_uart_cfg->flow_pins]);
    ...
    item = variable_item_list_add(
        var_item_list, "Пин DE/RE", COUNT_OF(software_de_re), line_software_de_re_cb, app);
