...
void nfc_scene_mf_ultralight_unlock_warn_on_enter(void* context) {
    ...
    if((type == MfUltralightAuthTypeReader) || (type == MfUltralightAuthTypeManual)) {
        // Build dialog text
        FuriString* password_str = furi_string_alloc_set_str("Разблокировать карту\nпаролем: ");
        ...
        dialog_ex_set_right_button_text(dialog_ex, "Продолжить");

        if(type == MfUltralightAuthTypeReader) {
            notification_message(nfc->notifications, &sequence_set_green_255);
        }
    } else {
        dialog_ex_set_header(dialog_ex, "Опасное действие!", 64, 4, AlignCenter, AlignTop);
        dialog_ex_set_text(
            dialog_ex, "Неверный пароль\nможет заблок.\nкарту.", 4, 18, AlignLeft, AlignTop);
