...
void nfc_render_iso14443_3b_info(
    ...
    if(format_type == NfcProtocolFormatTypeFull) {
        ...
        furi_string_cat_printf(str, "Тип: ISO 14443-%c (NFC-B)\n", iso_type);
    }
    ...
    furi_string_cat(str, "Макс. кадр: ");

    const uint16_t max_frame_size = iso14443_3b_get_frame_size_max(data);
    if(max_frame_size != 0) {
        furi_string_cat_printf(str, "%u байт\n", max_frame_size);
    } else {
        furi_string_cat(str, "? (RFU)\n");
    }

    const double fwt = iso14443_3b_get_fwt_fc_max(data) / 13.56e6;
    furi_string_cat_printf(str, "Макс. ожидание: %4.2g с\n", fwt);

    const char* nad_support_str =
        iso14443_3b_supports_frame_option(data, Iso14443_3bFrameOptionNad) ? "" : "не ";
    furi_string_cat_printf(str, "NAD: %sподдерж.\n", nad_support_str);

    const char* cid_support_str =
        iso14443_3b_supports_frame_option(data, Iso14443_3bFrameOptionCid) ? "" : "не ";
    furi_string_cat_printf(str, "CID: %sподдерж.", cid_support_str);
