...
// quick-action storage -- it lists the same four slots that already exist,
// after a fixed Archive entry (Archive's stock Down-short shortcut is now
// Control Center, and the stock firmware has no other way to open it).
#define DESKTOP_FAVORITES_ARCHIVE_INDEX FavoriteAppNumber
...
void desktop_scene_favorites_on_enter(void* context) {
    ...
    submenu_add_item(
        submenu,
        "Archive",
        DESKTOP_FAVORITES_ARCHIVE_INDEX,
    ...
    for(size_t i = 0; i < FavoriteAppNumber; i++) {
        ...
        FuriString* label = furi_string_alloc_printf(
            ...
            (strlen(path) == 0) ? "Apps Menu" : furi_string_get_cstr(app_name));
        submenu_add_item(
            submenu,
...
bool desktop_scene_favorites_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == DESKTOP_FAVORITES_ARCHIVE_INDEX) {
