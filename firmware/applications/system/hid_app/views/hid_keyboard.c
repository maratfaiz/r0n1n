...
static void hid_keyboard_draw_callback(Canvas* canvas, void* context) {
    ...
    if(!model->connected) {
        ...
        elements_multiline_text_aligned(canvas, 17, 3, AlignLeft, AlignTop, "Клавиат.");

        canvas_draw_icon(canvas, 68, 3, &I_Pin_back_arrow_10x8);
        canvas_set_font(canvas, FontSecondary);
        elements_multiline_text_aligned(canvas, 127, 4, AlignRight, AlignTop, "держи");

        elements_multiline_text_aligned(
            canvas, 4, 60, AlignLeft, AlignBottom, "Ожидание подключения...");
