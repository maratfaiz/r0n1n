...
void hid_scene_unpair_on_enter(void* context) {
    ...
    dialog_ex_set_header(app->dialog, "Забыть устройство?", 64, 3, AlignCenter, AlignTop);
    dialog_ex_set_left_button_text(app->dialog, "Назад");
    dialog_ex_set_right_button_text(app->dialog, "Забыть");

    // Un-pair success popup view
    popup_set_icon(app->popup, 48, 6, &I_DolphinDone_80x58);
    popup_set_header(app->popup, "Готово", 14, 15, AlignLeft, AlignTop);
